...
def convert_ogg_to_wav(ogg_path: Union[str, Path]) -> Path:
    ...
    return convert_audio_to_wav(ogg_path)


def convert_audio_to_wav(audio_path: Union[str, Path]) -> Path:
    """
    ...
    try:
        ...
        audio = AudioSegment.from_file(str(audio_path), format=suffix or None)
